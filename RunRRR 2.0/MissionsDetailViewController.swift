...
class MissionsDetailViewController: UIViewController,UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func isCameraButton(){
        ...
        Alamofire.request("\(CONFIG.API_PREFIX.ROOT)/member/read",method: .get, parameters: parameterSent).validate().responseJSON{ response in
    ...
    @objc func choosePhoto() {
        if (mission?.check != 1) && (mission?.check != 3){
            let cameraViewController = ALCameraViewController.CameraViewController(croppingParameters: CroppingParameters(isEnabled: false)) { [weak self] image, asset in
    ...
    func setupMissionDetail(){
        ...
        let missionImage: UIImageView = {
            ...
            if let remoteMissionImage = mission?.missionImageURL{
                let missionImageURL:URLConvertible = "\(CONFIG.API_PREFIX.ROOT)/download/img/" + remoteMissionImage
        ...
        let missionReportImage: UIImageView = {
            ...
            if self.missionReportImage2Show == nil{
                if let remoteImage = mission?.imageURL{
                    let imageURL:URLConvertible = "\(CONFIG.API_PREFIX.ROOT)/download/img/" + remoteImage
